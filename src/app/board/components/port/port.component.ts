...
export class PortComponent implements OnInit {
  ...
  constructor(private placingService : PlacingService) 
  {
	...
	ngOnInit(): void {
    this.LogicPort = this.LogicParent.getPort(this.IsOutput);//new Port(this.LogicParent);
    ...
    if(this.placingService.isConnecting){
      ...
      this.board.onmousemove = (event) => {
        if(this.placingService.canMoveConnection){
          if(!this.placingService.isConnecting) this.placingService.startConnecting(this);
